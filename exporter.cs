...
namespace esapi
{
    class exporter
    {
        ...
        public static void export_image(VMS.TPS.Common.Model.API.Image img, string dir, string name)
        {
        ...
        public static void export_image_mhd(VMS.TPS.Common.Model.API.Image img, string dir, string name)
        {
            string fn = $"export_image_mhd(img={_Id(img)},dir={dir}, name={name})";
            print(fn);
            ...
            if (!filesystem.file_exists(path_mhd))
            {
                StringBuilder sb = new StringBuilder();
                {
                    ...
                    sb.AppendLine("ElementDataFile = img.raw");
            ...
            for (uint z = 0; z < size[2]; z++)
            {
                ...
                for (uint y = 0; y < size[1]; y++)
                {
                    for (uint x = 0; x < size[0]; x++)
                    {
                        ...
                        bufferSave[z * (size[0] * size[1]) + y * (size[0]) + x] = (Int16)value;
                    } // x
                } // Y
        ...
        public static void export_structure_as_mask_image(VMS.TPS.Common.Model.API.Image img, Structure s, string dir, string name)
        {
            ...
            if (!filesystem.file_exists(path_mhd))
            {
                ...
                sb2.AppendLine("CompressedData = False");
